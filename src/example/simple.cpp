...
int main() {
    ...
    std::shared_ptr<FiniteElement2D> element1(new TriangleElement2D(n1, n3, n2, *material));
    std::shared_ptr<FiniteElement2D> element2(new TriangleElement2D(n2, n3, n4, *material));
    ...
    std::shared_ptr<NodeForce2D> load = std::make_shared<ConcentratedLoad2D>(n2, Axis2D::Y, -1000);
